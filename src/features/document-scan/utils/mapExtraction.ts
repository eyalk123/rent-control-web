...
import type { FieldNote, LeaseExtraction, ProvenanceItem, ReviewItem } from '../types';
...
export function mapExtraction(extraction: LeaseExtraction): MappedExtraction {
  ...
  const r = extraction.renter;

  const notes = new Map<string, FieldNote>();
  for (const n of extraction.notes ?? []) notes.set(`${n.section}.${n.field}`, n);
  ...
  const renterPrefill: Partial<RenterFormValues> = {};
  const renterReview: ReviewItem[] = [];
  const renterProvenance: ProvenanceItem[] = [];
  for (const a of renterAssigns) {
    const v = a.get();
    if (v === undefined) continue;
    (renterPrefill as Record<string, unknown>)[a.key] = v;
    const note = notes.get(`renter.${a.field}`);
    const source = note?.source_text ?? null;
    renterProvenance.push({ formKey: a.key as string, labelKey: a.i18n, prefilledValue: v, source });
    if (note) renterReview.push({ field: a.i18n, formKey: a.key as string, value: v, source, confidence: note.confidence });
  }
  ...
  if (r.rent_escalation_mode === 'custom' && r.lease_years && r.lease_years.length) {
    renterPrefill.leaseYears = r.lease_years.map((ly) => ({ amount: String(ly.amount), type: ly.type }));
  }
  if (r.extra_contacts && r.extra_contacts.length) {
    renterPrefill.extraContacts = r.extra_contacts.map((c) => ({ name: c.name, phone: c.phone }));
  }

  return { propertyPrefill, propertyReview, propertyProvenance, renterPrefill, renterReview, renterProvenance };
}
